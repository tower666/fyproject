...
from django.db.models import Q  # django内置的给搜索用的！
from django.shortcuts import get_object_or_404
from django.shortcuts import redirect
from django_comments.models import Comment
from django_comments import models as comment_models

def make_paginator(objects, page, num=5):  # 列表(我对谁分页)，查看第几页， 每页显示多少
    paginator = Paginator(objects, num)
    try:
        object_list = paginator.page(page)
    except PageNotAnInteger:  # 如果用户请求的不是整数页
        object_list = paginator.page(1)  # 我就给他第一页
    except EmptyPage:  # 如果页码超过了我的页码
        object_list = paginator.page(paginator.num_pages)
    return object_list, paginator


def pagination_data(paginator, page):  # 直接拷贝的。拿来用
    """
    用于自定义展示分页页码的方法
...
def detail(request, blog_id):
    ...
    entry.toc = md.toc  # 制作目录
    entry.increase_visiting()

    comment_list = list()  # 定义一个列表装顶级评论的

    def get_comment_list(comments):
        for comment in comments:
            comment_list.append(comment)   # 把顶级评论加到列表里
            children = comment.child_comment.all()
            # 这一条怎么来的 是django_comments里面的abstract.py里面的一个字段。里面的 related_name='child_comment'
            # parent_comment = models.ForeignKey('self', default=None, null=True, blank=True, related_name='child_comment',on_delete=models.CASCADE)
            if len(children) > 0:     # 表示有子评论
                get_comment_list(children)    # 递归调用子评论
...
def category(request, category_id):  #
    # c = models.Category.objects.get(id=category_id)  # 获取分类
    c = get_object_or_404(models.Category, id=category_id)
    entries = models.Entry.objects.filter(category=c)

    page = request.GET.get('page', 1)  # 需要分页
    entry_list, paginator = make_paginator(entries, page)  # 需要分页
    page_data = pagination_data(paginator, page)  # 需要分页

    return render(request, 'blog/index.html', locals())


def tag(request, tag_id):  #
    # t = models.Tags.objects.get(id=tag_id)  # 获取分类
    t = get_object_or_404(models.Tags, id=tag_id)
    if t.name == '全部':
        entries = models.Entry.objects.all()
    else:
...
def search(request):
    keyword = request.GET.get('keyword', None)  # base.html
...
def login(request):
    ...
    access_token_url = 'https://api.weibo.com/oauth2/access_token?client_id=%s&client_secret=%s&grant_type=authorization_code&redirect_uri=http://www.yoursite.com/login&code=%s'\
...
def logout(request):
    ...
    # request.session.flush()   # 如果用这个方法就会把用户全部的信息删掉。有些人想保留的话就只能一项一项去del
