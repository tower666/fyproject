...
class CommentDetailsForm(CommentSecurityForm):
    ...
    """
    # name = forms.CharField(label=pgettext_lazy("Person name", "Name"), max_length=50)  # 此处注销
    # email = forms.EmailField(label=_("Email address"))   # 此处注销
    # url = forms.URLField(label=_("URL"), required=False)   # 此处注销
    # Translators: 'Comment' is a noun here.

    comment_title = forms.CharField(max_length=256, label=_('标题'))   # 增加的字段
    parent_id = forms.IntegerField(min_value=-1, label=_('父亲评论的id'))   # 增加的字段  min_value=-1表示没有父评论
    level = forms.IntegerField(min_value=0, label=_('层级'))   # 增加的字段
    ...
    def get_comment_create_data(self, site_id=None):
        ...
        if parent_id <=0:   # 如果没有评论           #
            parent = None                          #
            _level = self.cleaned_data['level']  #
        else:  #
            parent = get_model().objects.get(id=parent_id)  #
            _level = self.cleaned_data['parent_id'] + 4  # +4 用来控制缩进让父评论和子评论错开！

        return dict(
            content_type=ContentType.objects.get_for_model(self.target_object),
            object_pk=force_str(self.target_object._get_pk_val()),
            # user_name=self.cleaned_data["name"],   # 此处注释
            # user_email=self.cleaned_data["email"],   # 此处注释
            # user_url=self.cleaned_data["url"],   # 此处注释
            comment_title = self.cleaned_data['comment_title'],  # 此处添加
            parent_comment=parent,   # 此处添加
            level = _level,  # 此处添加
